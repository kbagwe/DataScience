...
#For creating sidebox displaying mean value 
#when a checkbox in sidebar checked:
# (1) print the mean daily production for the sources selected

if st.sidebar.checkbox("Show Mean Daily Production "):
    st.write("Mean Daily Production of " , option , " is displayed below")
    st.write(df[option].mean(axis =0))
# #        (2) show a bar chart using the mean daily production for the sources selected
# #        (check the Streamlit API to find how a bar chart can be created)
    st.write("Bar Chart of Mean Daily Production of" , option , "is displayed below")
    st.bar_chart(df[option].mean())
